...
object RowExtractorTest{
  ...
  val inputData = "/Users/franklinzhao/Documents/ITprojects/java/cobrix/examples/example_data/raw_data/file1.bin"
  ...
  val byteData = getBinaryData(inputData)
  println("byteData:::"+byteData.length)

  val startOffset: Int = 0
  val schemaPolicy: SchemaRetentionPolicy = SchemaRetentionPolicy.KeepOriginal //CollapseRoot

  def main(args: Array[String]): Unit = {
    val row = RecordExtractors.extractRecord(copybook.ast, bytes, startOffset, schemaPolicy, handler = new SimpleRecordHandler())
